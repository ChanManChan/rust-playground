...
fn main() {
    App::new()
        ...
        .add_systems(FixedUpdate, (move_paddle, apply_velocity, check_ball_collisions.after(apply_velocity)))
        .run();
}
...
fn move_paddle(input: Res<Input<KeyCode>>, time_step: Res<Time<Fixed>>, mut query: Query<&mut Transform, With<Paddle>>) {
    ...
    let mut new_x = paddle_transform.translation.x + direction * PADDLE_SPEED * time_step.delta_seconds();
